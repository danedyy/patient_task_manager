...
/// Reads and writes the confirmed-server layer, speaking to the domain
/// [PatientTask] so the repository never sees Drift row types.
@DriftAccessor(tables: [ConfirmedTasks])
class TaskDao extends DatabaseAccessor<AppDatabase> with _$TaskDaoMixin {
  TaskDao(super.db);

  Stream<List<PatientTask>> watchAll() => select(
    confirmedTasks,
  ...
  /// Version-guarded upsert of a whole page in one transaction, so the reactive
  /// [watchAll] emits **once** for the page. Upserting row-by-row outside a
  /// transaction makes `watch()` fire per row, so paginated tasks trickle into
  /// the UI one at a time. Reuses [upsertFromServer], so the per-row `version`
  /// guard is identical; Drift coalesces the notifications until commit.
  Future<void> upsertAllFromServer(List<PatientTask> tasks) =>
      transaction(() async {
        for (final task in tasks) {
          await upsertFromServer(task);
        }
      });
